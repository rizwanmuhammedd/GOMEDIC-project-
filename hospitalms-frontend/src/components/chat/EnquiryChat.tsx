...
export const EnquiryChat = ({ isOpen, onClose, onUnreadChange }: { isOpen: boolean, onClose: () => void, onUnreadChange?: (count: number) => void }) => {
    ...
    // Internal state for Patient floating button
    ...
    const isPatientChatOpenRef = useRef(isPatientChatOpen);
    useEffect(() => { isPatientChatOpenRef.current = isPatientChatOpen; }, [isPatientChatOpen]);

    // 1. Initial Load of Unread Count from server
    useEffect(() => {
        const fetchUnread = async () => {
            try {
                const res = await chatApi.getUnreadCount();
                setPatientUnreadCount(res.data.count);
            } catch (err) {
                console.error('Failed to fetch unread chat count', err);
            }
        };
        if (isAuthenticated && user) fetchUnread();
    }, [isAuthenticated, user]);

    // 2. Mark as read in Backend logic
    const markChatAsRead = async () => {
        try {
            if (user?.role === 'Patient') {
                await chatApi.markHospitalAsRead(user.id.toString());
                setPatientUnreadCount(0);
            } else if (user?.role === 'Receptionist' && activePatientId) {
                await chatApi.markAsRead(activePatientId);
                setPatientList(prev => prev.map(p => p.id === activePatientId ? { ...p, unreadCount: 0 } : p));
            }
        } catch (err) {
    ...
    // Auto-mark as read when chat is opened (Patient)
    useEffect(() => {
        const showChat = isPatientChatOpen || isOpen;
        if (showChat && user?.role === 'Patient') {
            markChatAsRead();
        }
    }, [isPatientChatOpen, isOpen, user]);

    // Auto-mark as read when patient selected (Receptionist)
    useEffect(() => {
        if (activePatientId && user?.role === 'Receptionist') {
            markChatAsRead();
        }
    }, [activePatientId, user]);

    // 3. Propagate unread status to Header Badge
    useEffect(() => {
        if (!onUnreadChange || !user) return;
        if (user.role === 'Receptionist') {
            const count = patientList.filter(p => (p.unreadCount || 0) > 0).length;
            onUnreadChange(count);
        } else if (user.role === 'Patient') {
            onUnreadChange(patientUnreadCount);
        }
    }, [patientList, patientUnreadCount, user, onUnreadChange]);
    ...
    // Fetch specific patient history for receptionist
    useEffect(() => {
    ...
    const signalR = useSignalR([
        {
            ...
            handler: (enquiry: any) => {
                const isMyMessage = user?.role === 'Receptionist' 
                    ...
                    : (enquiry.isFromPatient && enquiry.patientId === user.id.toString());

                // Increment local unread count if chat is closed (Patient)
                if (user?.role === 'Patient' && !enquiry.isFromPatient && enquiry.patientId === user.id.toString()) {
                    const showChat = isPatientChatOpenRef.current || isOpenRef.current;
                    if (!showChat) {
                        setPatientUnreadCount(prev => prev + 1);
                    } else {
                        chatApi.markHospitalAsRead(user.id.toString()).catch(() => {});
                    }
                }

                // Update unread count per patient (Receptionist)
                if (user?.role === 'Receptionist' && enquiry.isFromPatient) {
                    if (activePatientIdRef.current !== enquiry.patientId) {
                        setPatientList(prev => {
                            const idx = prev.findIndex(p => p.id === enquiry.patientId);
                            if (idx === -1) {
                                return [{ 
                            ...
                            return updated.sort((a, b) => new Date(b.latestMessage).getTime() - new Date(a.latestMessage).getTime());
                        });
                    } else {
                        chatApi.markAsRead(enquiry.patientId).catch(() => {});
                    }
                }
    ...
    }, [chats, isOpen, isPatientChatOpen, activePatientId]);
    ...
    // Receptionist logic (Navbar Modal)
    if (user?.role === 'Receptionist') {
        if (!isOpen) return null;
        return (
            <Modal isOpen={isOpen} onClose={onClose} title="Patient Enquiry Desk" size="xl">
                <div className="flex h-[600px] bg-white overflow-hidden -m-6">
                    ...
                    <div className="flex border-b border-zinc-100 h-full w-full">
    ...
    // Patient logic (Floating button)
    if (user?.role === 'Patient') {
        const showChat = isPatientChatOpen || isOpen;
        return (
            <>
                <button
                    onClick={() => setIsPatientChatOpen(!isPatientChatOpen)}
                ...
                >
                    ...
                    {patientUnreadCount > 0 && !showChat && (
                        <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-white shadow-sm animate-bounce">
                            {patientUnreadCount > 9 ? '9+' : patientUnreadCount}
                        </span>
                    )}
                </button>
                ...
                {showChat && (
                    <div className="fixed bottom-24 right-6 w-80 sm:w-96 h-[500px] bg-white border border-zinc-200 rounded-[24px] shadow-2xl overflow-hidden flex flex-col z-[999] animate-in slide-in-from-bottom-4 duration-300">
                        <div className={`p-4 ${isBlockedByAdmin ? 'bg-red-500' : 'bg-emerald-500'} text-white flex items-center justify-between gap-3`}>
                            ...
                            <button onClick={() => { setIsPatientChatOpen(false); onClose(); }} className="p-1 hover:bg-white/10 rounded-full transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#FDFDFD]">
                            {isBlockedByAdmin ? (
                                <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                                    ...
                                    <Button variant="secondary" size="sm" className="mt-6" onClick={() => setIsPatientChatOpen(false)}>Close Chat</Button>
                                </div>
                            ) : (
                                <>
                                    {chats.length === 0 && (
